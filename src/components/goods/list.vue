<template>
  <div class="list">
    ...
    <el-card>
      ...
      <!-- 表格区域 -->
      <el-table :data="goodsList" style="width: 100%" border stripe row-key="id">
        ...
        <el-table-column label="创建时间" width="180px">
          <template v-slot='slotProps'>{{slotProps.row.add_time|dataFormat}}</template>
        </el-table-column>
        <el-table-column label="操作" width="180px">
          <template v-slot='slotProps'>
            <el-button type="primary" icon='el-icon-edit' size='mini'></el-button>
            <el-button type="danger" icon='el-icon-delete' size="mini" @click='delGoods(slotProps.row.goods_id)'></el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页区域 -->
